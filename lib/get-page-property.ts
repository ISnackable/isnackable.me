import type { Block, DateFormat, ExtendedRecordMap } from 'notion-types';
  ...
  try {
    ...
    if (collection) {
      ...
      switch (type) {
        ...
        case 'date': {
          const property = block.properties[propertyId] as [
            ['‣', [DateFormat]],
          ];
        ...
        // @ts-expect-error Custom image property
        case 'image': {
        }

        case 'person': {
          const property = block.properties[propertyId] as [['‣', [string]]];
          const userId = property[0][1][0][1];
