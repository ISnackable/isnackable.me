...
export const loadLanguage = (lang: string) => {
  ...
  // check to match the lang to the language object
  if (lang in languages) {
    prismLang = lang;
  } else {
    // if not found, try to check if the associated filenames and extnames
    for (const [alias, associations] of Object.entries(languages)) {
      if (
        associations.extnames.includes(lang) ||
        associations.filenames.includes(lang)
      )
        prismLang = alias;
    }
  }
