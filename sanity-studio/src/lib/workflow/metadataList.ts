...
function getWorkflowMetadataListDataEvent$(workflowTypes) {
  const documentListSnapshotEvent$ = getDocumentQuery$(DOCUMENT_LIST_QUERY, {
    ...
    map((snapshot) => ({ type: "document.snapshot", snapshot, timestamp: 0 }))
  );

  const documentListMutationsEvent$ = getDocumentMutations$(
    DOCUMENT_LIST_FILTER,
    {
      workflowTypes,
    }
  );
  ...
  const updatedDocumentListSnapshotEvent$ = documentListMutationsEvent$.pipe(
    filter((mutationEvent: any) => mutationEvent.type !== "welcome"),
    switchMap((mutationEvent: any) => {
  ...
  const metadataListMutationsEvent$ = getDocumentMutations$(
    METADATA_LIST_FILTER,
    {
      type: "workflow.metadata",
    }
  );
  ...
  const updatedMetadataListSnapshotEvent$ = metadataListMutationsEvent$.pipe(
    ...
    switchMap((mutationEvent: any) => {
      ...
      return getDocumentQuery$(METADATA_LIST_QUERY, {
        type: "workflow.metadata",
      }).pipe(
        map((snapshot) => ({
          type: "snapshot",
