...
export function BoardTool() {
  ...
  const documentIdsWithoutMetadata = documentIds.filter(
    (id) => !metadataDocumentIds.includes(id)
  );
  ...
  const bindDrag = useDrag(({ args, down, xy, movement }) => {
    ...
    const currentColumn = Math.round(cursorX / columnWidth);
          ...
          <Box paddingY={5} paddingX={3}>
            <Card shadow={1} padding={4} style={{ textAlign: "center" }}>
              <Button
                tone="primary"
                onClick={() =>
                  metadataList.importDocuments(documentIdsWithoutMetadata)
                }
              >
                        ...
                        <div key={item._id}>
                          <DocumentCard
                            ...
                            onAssigneeRemove={(userId) =>
                              metadataList.removeAssignee(
                                item.documentId,
                                userId
                              )
                            }
                            onAssigneesClear={() =>
                              metadataList.clearAssignees(item.documentId)
                            }
                          />
                        </div>
                      ))}
